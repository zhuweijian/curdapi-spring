...
import com.zwj.curdapispring.crudapi.SQL.Statics;
...
import static com.zwj.curdapispring.crudapi.RequestHandler.Actions.*;
import static com.zwj.curdapispring.crudapi.SQL.Statics.*;
...
public class RequestHandler {
    ...
    private RequestHandler(Connection link, HttpServletRequest req, ApiConfig apiConfig) throws SQLException, IOException, ClassNotFoundException {
        ...
        String tableName = request[0].replaceAll("[^a-zA-Z0-9_]+", ""); //这里是空值
     ...
     */
    public static void handle(HttpServletRequest req, StringBuilder sb, ApiConfig apiConfig) throws SQLException, IOException, ClassNotFoundException {
    ...
    // todo 由method决定actiong，改为 所有请求都是 post， 路径 /tableName/action
    ...
    private LinkedList<Map<String, Object>> processJsonObjectResults(
            ResultSet rs, StringBuilder sb, Collection<String> columns, TableMeta tableMeta,
            ...
            for (TableMeta subTableMeta : tableMeta.getReferencedTables().values()) {
                ...
                try (ResultSet resultSet = prepareStatement(sql).executeQuery()) {
                    LinkedList<Map<String, Object>> relatedRecords = processJsonObjectResults(
                            resultSet, sb, columnsList, subTableMeta, collectIds, subRelation);
                    if (subRelation == null) {
                        sb.append(","); //list non-related table objects sequentially
                    } else {
        ...
        if (relation == null) {
            sb.append(String.format("\"%s\":", tableMeta.getName()));
            sb.append(gson.toJson(records));
        }
        return records;
    }
    ...
    private void streamJsonObjectResults(ResultSet rs, StringBuilder sb, TableMeta tableMeta)
...
//            case "NUMBER":
            ...
            case BINARY:
                ...
                value = bytes != null ? com.zwj.curdapispring.crudapi.Base64.encodeToString(bytes, com.zwj.curdapispring.crudapi.Base64.DEFAULT).trim() : null;
                break;
            case TIME:
    ...
    private void handleRequest(StringBuilder sb) throws SQLException, ClassNotFoundException {
...
//           process multiple row data
        ...
        action = config.before(action, databaseName, table, ids, input);
        if (isReadOnly()) {
            doReadonlyActions(sb, columnsList, ids);
        } else {
            if (batch.isEmpty()) {
                try {
                    if (ids == null) {
            ...
            if (action == HEADERS) {
                sb.append("[\"Access-Control-Allow-Headers: Content-Type, X-XSRF-TOKEN\",\"Access-Control-Allow-Methods: OPTIONS, GET, PUT, POST, DELETE, PATCH\",\"Access-Control-Allow-Credentials: true\",\"Access-Control-Max-Age: 1728000\"]");
            } else {
                if (batch.isEmpty()) {
                    if (sql == null) {
                ...
                try {
                    ...
                    link.commit();
                    sb.append(results.size() == 1 ? gson.toJson(results.get(0)) : gson.toJson(results));
                } catch (SQLException ex) {
                    link.rollback();
    ...
    private void doReadonlyActions(StringBuilder sb, List<String> columnsList, String... ids) throws SQLException, ClassNotFoundException {
        ...
        if (parameters.containsKey(ID_KEY)) {
            ...
            try (ResultSet rs = prepareStatement(sql).executeQuery()) {
                sb.append(preamble);
                streamJsonObjectResults(rs, sb, tableMetaMap.get(table));
            }
            if (!preamble.isEmpty()) {
                sb.append("]");
            }
            ...
            try (ResultSet rs = prepareStatement(sql).executeQuery()) {
                ...
                if (hasIncludedTables) {
                    processJsonObjectResults(rs, sb, columnList, tableMetaMap.get(table), collectIds, null);
                } else {
            ...
            try (ResultSet rs = prepareStatement(sql).executeQuery()) {
                sb.append("{");
                streamRecords(sb, columnsList, collectIds, rs, resultCount, topTable);
            }
            streamRelatedTables(sb, collectIds, topTable);
            sb.append("}");
        }
    }
    ...
    private void streamRelatedTables(StringBuilder sb, Map<String, Set<Object>> collectIds, TableMeta tableMeta) throws SQLException {
        for (TableMeta mt : tableMeta.getReferencedTables().values()) {
            ...
            try (ResultSet resultSet = prepareStatement(sql).executeQuery()) {
                sb.append(",");
                streamRecords(sb, columnsList, collectIds, resultSet, null, mt);
            }
            streamRelatedTables(sb, collectIds, mt);
        }
    }

    private void streamRecords(StringBuilder sb, List<String> columnsList, Map<String, Set<Object>> collectIds,
            ResultSet rs, Integer resultCount, TableMeta tableMeta) throws SQLException {
        String relations = tableMeta.getRelationsJson();
        sb.append(String.format("\"%s\":{%s\"columns\":%s,\"records\":[", tableMeta.getName(),
                relations,
        ...
        sb.append("]");
        if (resultCount != null) {
            sb.append(String.format(",\"results\":%d", resultCount));
        }
        sb.append("}");
    }
    ...
    private SQL prepareSql(List<String> columnsList) {
        ...
        switch (action) {
            ...
            case LIST:
                ...
                break;
            case UPDATE:
                sql = Statics.UPDATE(table);
                break;
            case CREATE:
                sql = INSERT().INTO(table);
                break;
            case DELETE:
                sql = Statics.DELETE().FROM(table);
                break;
            case HEADERS:
                sql = WITH();
                break;
    ...
    private Object validateInput(Map<String, Object> input, Map<String, String> typeMap, String key) {
        ...
        if (result != null) {
            if (isBinaryColumn(key, typeMap)) {
                result = com.zwj.curdapispring.crudapi.Base64.decode(result.toString(), Base64.DEFAULT);
